...
async def start(self, session):
    data = await create.start(session, "towers", {"betAmount": self.bet_amt, "difficulty": self.difficulty})
    if data:
        ...
        while True:
            ...
            click_data = await click.start(session, "towers", {'cashout': False, 'tile': _click, 'towerLevel': level})

            if click_data.get("exploded"):
                cprint.lost(f"Clicked a wrong tile..")
                await _continue._next(self, session, tower, True)
                lost = True
                break
        ...
        if not lost:
            await _continue._next(self, session, tower)
